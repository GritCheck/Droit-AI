...
export function ResAvailable({ title, subheader, chart, sx, ...other }: Props) {
  const theme = useTheme();

  const total = sumBy(chart.series, (series) => series.value);

  const chartSeries = (chart.series.filter((i) => i.label === 'Hate/Bias Blocked')[0].value / total) * 100;
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { offsetY: -12 },
          value: { offsetY: 6 },
          total: { label: 'Items', formatter: () => fNumber(total) },
        },
      },
    },
    ...chart.options,
          ...
          >
            <Box
              sx={{
                ...
                ...(item.label === 'Hate/Bias Blocked' && { bgcolor: chartColors[1] }),
              }}
            />
